...
#SBATCH --job-name Base-CenterNet2-AdamW

### Logging
#SBATCH --output=/scratch/cluster/jozhang/logs/slurm/base_ct2_adamw_%j.out # Name of stdout output file (%j expands to jobId)
#SBATCH --error=/scratch/cluster/jozhang/logs/slurm/base_ct2__adamw_%j.err  # Name of stderr output file (%j expands to jobId)
...
python train_net.py --config-file configs/Base-CenterNet2.yaml \
    --num-gpus 4                                 \
    OUTPUT_DIR /scratch/cluster/jozhang/logs/ct2/Base-CenterNet2-AdamW \
    SOLVER.USE_CUSTOM_SOLVER True \
