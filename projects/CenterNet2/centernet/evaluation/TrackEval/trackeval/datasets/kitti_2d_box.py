...
class Kitti2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.label_suffix = ''
        if self.config['SPLIT_TO_EVAL'] not in ['train', 'val', 'validation', 'test']:
            self.label_suffix = f'_{self.config["SPLIT_TO_EVAL"]}'
            print(f'Special SPLIT_TO_EVAL. Looking for labels at {self.gt_fol}/label_02{self.label_suffix}')
            ...
            for row in reader:
                if len(row) >= 4:
                    ...
                    if not self.data_is_zipped:
                        curr_file = os.path.join(self.gt_fol, 'label_02' + self.label_suffix, seq + '.txt')
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            zip_file = None
            if is_gt:
                file = os.path.join(self.gt_fol, 'label_02' + self.label_suffix, seq + '.txt')
            else:
