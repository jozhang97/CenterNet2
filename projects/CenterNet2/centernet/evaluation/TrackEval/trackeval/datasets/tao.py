...
class TAO(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'ORACLE': None   # None, track_class, track, class
        }
    ...
    def __init__(self, config=None):
        ...
        ann_path = os.path.join(self.gt_fol, gt_dir_files[0])
        with open(ann_path) as f:
            self.gt_data = json.load(f)

        if config['ORACLE']:
            import sys
            sys.path.insert(0, '/u/jozhang/code/motion3d')
            from external.tao.tao.toolkit.tao.tao import Tao as TaoDataset
            from external.tao.tao.toolkit.tao.results import TaoResults
            from external.tao.tao.utils.evaluation import apply_oracle
            self.tao = TaoDataset(ann_path)
        ...
        if self.config['CLASSES_TO_EVAL']:
            ...
            expected_missing_classes = set(self.config['CLASSES_TO_EVAL']) - set(self.valid_classes)
            if expected_missing_classes:
                print(f'Removing the following invalid classes from CLASSES_TO_EVAL {expected_missing_classes}')
                self.class_list = [cls.lower() for cls in self.config['CLASSES_TO_EVAL'] if cls.lower() in self.valid_classes]
        ...
        for tracker in self.tracker_list:
            ...
            if config['ORACLE']:
                ...
                assert oracle_type in ['track', 'class', 'track_class']
                logging.warning(f'Applying {oracle_type} oracle to tracker {tracker}')
                ...
                results = TaoResults(self.tao, curr_data)
                results = apply_oracle(self.tao, results, oracle_cfg, category_agnostic=False)
                curr_data = results.dataset['annotations']
